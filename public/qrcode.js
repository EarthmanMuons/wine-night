// Minimal QR code generator (byte mode, ECC level L, auto-version). Self-contained.
// Renders to a <canvas>. Based on the public-domain qrcode-generator by Kazuhiko Arase.
...
(function () {
  ...
  QRPolynomial.prototype = {
    ...
    mod: function (e) {
      ...
      for (var i2 = 0; i2 < e.getLength(); i2++) num[i2] ^= QRMath.gexp(QRMath.glog(e.get(i2)) + ratio);
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    [4, 144, 116, 4, 145, 117], [17, 68, 42, 0, 69, 43], [17, 50, 22, 6, 51, 23], [19, 46, 16, 6, 47, 17],
    [2, 139, 111, 7, 140, 112], [17, 74, 46, 0, 75, 47], [7, 54, 24, 16, 55, 25], [34, 37, 13, 0, 38, 14],
    ...
    [17, 145, 115, 0, 146, 116], [10, 74, 46, 23, 75, 47], [10, 54, 24, 35, 55, 25], [19, 45, 15, 35, 46, 16],
    [17, 145, 115, 1, 146, 116], [14, 74, 46, 21, 75, 47], [29, 54, 24, 19, 55, 25], [11, 45, 15, 46, 46, 16],
    [13, 145, 115, 4, 146, 116], [14, 74, 46, 23, 75, 47], [44, 54, 24, 7, 55, 25], [59, 46, 16, 1, 47, 17],
  ];
  ...
  // ISO/IEC 18004 format-information values. These are not ordered the same
  // way as the human-readable level names.
  var QRErrorCorrectionBits = { L: 1, M: 0, Q: 3, H: 2 };
  var QRUtil = {
    PATTERN_POSITION_TABLE: [
      ...
      [6, 28, 54, 80, 106, 132, 158], [6, 32, 58, 84, 110, 136, 162], [6, 26, 54, 82, 110, 138, 166], [6, 30, 58, 86, 114, 142, 170], [6, 34, 62, 90, 118, 146, 174],
    ],
    ...
    getLengthInBits: function (mode, type) {
      if (mode === QRMode.MODE_NUMBER) return type < 10 ? 10 : 12;
      if (mode === QRMode.MODE_ALPHA_NUM) return type < 10 ? 9 : 11;
      if (mode === QRMode.MODE_8BIT_BYTE) return type < 10 ? 8 : 16;
      if (mode === QRMode.MODE_KANJI) return type < 10 ? 8 : 10;
    },
    getLostPoint: function (qrCode) {
      ...
      for (var col3b = 0; col3b < moduleCount; col3b++) {
        for (var row3b = 0; row3b < moduleCount - 6; row3b++) {
          if (qrCode.isDark(row3b, col3b) && !qrCode.isDark(row3b + 1, col3b) && qrCode.isDark(row3b + 2, col3b) && qrCode.isDark(row3b + 3, col3b) && qrCode.isDark(row3b + 4, col3b) && !qrCode.isDark(row3b + 5, col3b) && qrCode.isDark(row3b + 6, col3b)) {
            lostPoint += 40;
          }
        }
      }
      ...
      for (var col4 = 0; col4 < moduleCount; col4++) {
        for (var row4 = 0; row4 < moduleCount; row4++) {
          if (qrCode.isDark(row4, col4)) darkCount++;
        }
      }
  ...
  function QR8bitByte(data) {
    ...
    this.parsedData = this.parsedData.map(function (b) { return b & 0xff; });
  }
  ...
  function QRModel(typeNumber, errorCorrectionLevel) {
  ...
  QRModel.prototype = {
    ...
    makeImpl: function (test, maskPattern) {
      ...
      if (this.dataCache === null) this.dataCache = QRModel.createData(this.typeNumber, this.errorCorrectionLevel, this.dataList);
    ...
    setupTypeInfo: function (test, maskPattern) {
      var levelBits = QRErrorCorrectionBits[this.errorCorrectionLevel];
      if (levelBits === undefined) throw new Error("bad error correction level:" + this.errorCorrectionLevel);
      var data = (levelBits << 3) | maskPattern;
  ...
  QRModel.createData = function (typeNumber, errorCorrectionLevel, dataList) {
  ...
  function getTypeNumber(text) {
    // choose the smallest version whose 8bit-byte data capacity fits (ECC L)
    var CAPACITY = [0, 17, 32, 53, 78, 106, 134, 154, 192, 230, 271, 321, 367, 425, 458, 520, 586, 644, 718, 792, 858, 929, 1003, 1091, 1171, 1273, 1367, 1465, 1528, 1628, 1732, 1840, 1952, 2068, 2188, 2303, 2431, 2563, 2699, 2809, 2953];
    for (var i = 1; i < CAPACITY.length; i++) {
      if (text.length <= CAPACITY[i]) return i;
    }
    throw new Error("payload too long for QR");
  }

  function qrToMatrix(text) {
    var typeNumber = getTypeNumber(text);
    var qr = new QRModel(typeNumber, "L");
    qr.addData(text);
    qr.make();
    ...
    for (var row = 0; row < size; row++) {
      ...
      for (var col = 0; col < size; col++) modules[row][col] = qr.isDark(row, col);
    }
    return modules;
  }

  function qrToCanvas(text, canvas) {
    var modules = qrToMatrix(text);
    ...
    for (var row = 0; row < size; row++) {
      for (var col = 0; col < size; col++) {
        if (modules[row][col]) {
          ctx.fillRect((col + pad) * scale, (row + pad) * scale, scale, scale);
        }
      }
    }
    return canvas;
  ...
  window.WNQR = { toCanvas: qrToCanvas, toMatrix: qrToMatrix };
})();
